...
export const AssetModal = ({ onClick, assetId }) => {
    ...
    const { id, image, status, name, healthscore, sensors, unitId, assignedUserIds, model, specifications } = assetState
    ...
    const availableUsersId = newUsersArray.filter(val => !assignedUserIds?.includes(val));

    return (
        <div className='asset-modal' >
            {waitAsset ?
                <div>
                    <button className="asset-modal__close" onClick={onClick}><CloseOutlined /></button>
                    <div className="asset-modal__image-container">
                        <Image src={image} />
                    </div>
                    <Form className="asset-modal__form" initialValues={assetState} onFinish={onFinish} >
                        ...
                        <div className="asset-moda__form-column">
                            <Form.Item label="Assign Users" name="assignedUserIds">
                                {assignedUserIds.map((user) => {
                                    return (
                                        <Checkbox key={user} checked>{user}</Checkbox>
                                    )
                                })}
                                {availableUsersId.map((user) => {
                                    return (
                                        <Checkbox key={user} >{user}</Checkbox>
                                    )
                                })}
                            </Form.Item>
                            ...
                            </Form.Item><Form.Item label='RPM' name='specifications.rpm'>
                                <Input disabled placeholder={specifications.rpm} />
                            </Form.Item>
                            <Button className="asset-modal__button" type="primary" htmlType="submit">
                                Submit
                            </Button>
                        </div>
                    </Form>
